...
from werkzeug.utils import secure_filename
from ..utils.notificaciones_ticketing import (
    notificar_nuevo_ticket,
    notificar_ticket_asignado,
...
class Ticket(db.Model):
    ...
    IdUsuarioCreador = db.Column(db.Integer, db.ForeignKey('usuario.IdUsuario'), nullable=False)
    ...
    IdAsignadoA = db.Column(db.Integer, db.ForeignKey('usuario.IdUsuario'), nullable=True)
    ...
    FechaUltimaActualizacion = db.Column(db.DateTime, default=datetime.now, onupdate=datetime.now)
...
class HistorialTicket(db.Model):
    ...
    Descripcion = db.Column(db.Text)
    FechaCambio = db.Column(db.DateTime, default=datetime.now)
    def __repr__(self):
...
def dashboard_ti():
    try:
        metricas_raw = db.session.execute(text("SELECT * FROM v_metricas_ticketing")).fetchone()
        metricas = dict(metricas_raw._mapping) if metricas_raw else {}
        
        tickets_result = db.session.execute(text("SELECT * FROM v_tickets_activos LIMIT 100")).fetchall()
        tickets = [dict(row._mapping) for row in tickets_result]
        
        return render_template('ticketing/dashboard_ti.html', tickets=tickets, metricas=metricas)
...
def mis_tickets():
    try:
        tickets_result = db.session.execute(text("""
            SELECT * FROM v_tickets_por_usuario WHERE IdUsuarioCreador = :user_id
...
def nuevo_ticket():
    if request.method == 'GET':
        ...
        return render_template('ticketing/nuevo_ticket.html', categorias=categorias)
    ...
    try:
        ...
        result = db.session.execute(text("""
            ...
            ) VALUES (:user_id, :titulo, :descripcion, :categoria, 2, 1, 'Portal de Usuario')
        """), {
        ...
        # ✅ NOTIFICACIÓN: Nuevo ticket creado
        try:
            ...
            usuario_nombre = getattr(current_user, 'nombre', None) or \
                           getattr(current_user, 'Nombre', None) or \
                           getattr(current_user, 'nombre_completo', None) or \
                           'Usuario'
            
            ticket_info = {
                'IdTicket': id_ticket,
                'NumeroTicket': ticket_numero,
                'Titulo': titulo,
                'UsuarioCreador': usuario_nombre,
                'EmailCreador': current_user.Correo,
                'Prioridad': 'Media'
            }
            notificar_nuevo_ticket(ticket_info, url_base)
        except Exception as e:
            current_app.logger.error(f'Error enviando notificación de nuevo ticket: {str(e)}')
        
        socketio.emit('ticketing_update', {'tipo': 'nuevo_ticket', 'ticket_id': id_ticket})
...
def detalle_ticket(id_ticket):
    try:
        ...
        comentarios = [dict(row._mapping) for row in comentarios_result if not row._mapping['EsInterno'] or es_ti]
        ...
        return render_template('ticketing/detalle_ticket.html', ticket=ticket, comentarios=comentarios, 
                             adjuntos=adjuntos, historial=historial, es_ti=es_ti)
    except Exception as e:
...
def api_categorias():
    categorias = CategoriaTicket.query.filter_by(Activo=True).order_by(CategoriaTicket.Nombre).all()
    return jsonify([{
        'id': c.IdCategoria, 'nombre': c.Nombre, 'descripcion': c.Descripcion,
        'icono': c.Icono, 'color': c.Color
    } for c in categorias])
...
def api_prioridades():
    ...
    return jsonify([{
        'id': p.IdPrioridad, 'nombre': p.Nombre, 'descripcion': p.Descripcion,
        'color': p.Color, 'nivel': p.Nivel
...
def api_estados():
    ...
    return jsonify([{
        'id': e.IdEstado, 'nombre': e.Nombre, 'descripcion': e.Descripcion,
        'color': e.Color, 'es_final': e.EsEstadoFinal
    } for e in estados])
...
def api_tecnicos():
    try:
        tecnicos_result = db.session.execute(text("""
            SELECT IdUsuario, NombreCompleto, Correo FROM v_tecnicos_ti
        """)).fetchall()
        return jsonify([{'id': row[0], 'nombre': row[1], 'email': row[2]} for row in tecnicos_result])
    except Exception as e:
...
def asignar_ticket(id_ticket):
    try:
        ...
        query = "UPDATE Tickets SET IdAsignadoA = :tecnico, IdEstado = 2"
        ...
        try:
            url_base = request.url_root.rstrip('/')
            ticket_data_result = db.session.execute(text("""
                SELECT t.NumeroTicket, t.Titulo, p.Nombre as Prioridad
                FROM Tickets t
                LEFT JOIN PrioridadesTicket p ON t.IdPrioridad = p.IdPrioridad
                WHERE t.IdTicket = :id
            """), {'id': id_ticket}).fetchone()
            
            tecnico_result = db.session.execute(text("""
                SELECT NombreCompleto, Correo FROM usuario WHERE IdUsuario = :id
            """), {'id': id_tecnico}).fetchone()
            
            if ticket_data_result and tecnico_result:
                ticket_info = {
                    'IdTicket': id_ticket,
                    'NumeroTicket': ticket_data_result[0],
                    'Titulo': ticket_data_result[1],
                    'Prioridad': ticket_data_result[2]
                }
                tecnico_info = {'Nombre': tecnico_result[0], 'Email': tecnico_result[1]}
                notificar_ticket_asignado(ticket_info, tecnico_info, url_base)
        except Exception as e:
            current_app.logger.error(f'Error enviando notificación de asignación: {str(e)}')
        
        socketio.emit('ticketing_update', {'tipo': 'asignacion', 'ticket_id': id_ticket, 'tecnico_id': id_tecnico})
        
        ticket = db.session.execute(text("SELECT NumeroTicket FROM Tickets WHERE IdTicket = :id"), 
                                   {'id': id_ticket}).scalar()
...
def agregar_comentario(id_ticket):
    try:
        ...
        db.session.execute(text("""
            ...
            ) VALUES (:ticket, :usuario, :comentario, :interno, :oficial)
        """), {
            'ticket': id_ticket,
            'usuario': current_user.id,
            'comentario': comentario_texto,
            'interno': 1 if es_interno else 0,
            'oficial': 1 if tiene_permiso('Ticketing', 'editar') else 0
        })
        db.session.commit()
        ...
        try:
            url_base = request.url_root.rstrip('/')
            ticket_data_result = db.session.execute(text("""
                SELECT t.NumeroTicket, uc.Correo as EmailCreador, ua.Correo as EmailAsignado
                FROM Tickets t
                INNER JOIN usuario uc ON t.IdUsuarioCreador = uc.IdUsuario
                LEFT JOIN usuario ua ON t.IdAsignadoA = ua.IdUsuario
                WHERE t.IdTicket = :id
            """), {'id': id_ticket}).fetchone()
            ...
            if ticket_data_result:
                ticket_info = {
                    'IdTicket': id_ticket,
                    'NumeroTicket': ticket_data_result[0],
                    'EmailCreador': ticket_data_result[1],
                    'EmailAsignado': ticket_data_result[2]
                }
                # Obtener nombre del usuario actual
                usuario_nombre = getattr(current_user, 'nombre', None) or \
                               getattr(current_user, 'Nombre', None) or \
                ...
                notificar_nuevo_comentario(ticket_info, comentario_info, url_base)
        except Exception as e:
            current_app.logger.error(f'Error enviando notificación de comentario: {str(e)}')
...
def subir_adjunto(id_ticket):
    try:
        ...
        db.session.execute(text("""
            ...
            ) VALUES (:ticket, :usuario, :nombre, :original, :ruta, :mime, :tamano)
        """), {
            'ticket': id_ticket, 'usuario': current_user.id, 'nombre': nombre_archivo,
            'original': nombre_original, 'ruta': ruta_relativa, 
            'mime': archivo.content_type, 'tamano': tamano_kb
        })
        db.session.commit()
...
def cambiar_estado(id_ticket):
    try:
        ...
        estado_anterior_result = db.session.execute(text("""
            SELECT e.Nombre FROM Tickets t
            INNER JOIN EstadosTicket e ON t.IdEstado = e.IdEstado
            WHERE t.IdTicket = :id
        """), {'id': id_ticket}).scalar()
        
        db.session.execute(text("""
            UPDATE Tickets SET IdEstado = :estado WHERE IdTicket = :ticket
        """), {'estado': nuevo_estado, 'ticket': id_ticket})
        db.session.commit()
        ...
        try:
            url_base = request.url_root.rstrip('/')
            ticket_data_result = db.session.execute(text("""
                SELECT t.NumeroTicket, uc.Correo as EmailCreador, e.Nombre as EstadoNuevo
                FROM Tickets t
                INNER JOIN usuario uc ON t.IdUsuarioCreador = uc.IdUsuario
                INNER JOIN EstadosTicket e ON t.IdEstado = e.IdEstado
                WHERE t.IdTicket = :id
            ...
            if ticket_data_result:
                ...
                notificar_cambio_estado(ticket_info, estado_anterior_result, ticket_data_result[2], url_base)
                
                # Si el nuevo estado es "Resuelto", enviar notificación especial
                if ticket_data_result[2] == 'Resuelto':
                    ticket_resuelto_data = db.session.execute(text("""
                        SELECT NumeroTicket, Titulo FROM Tickets WHERE IdTicket = :id
                    """), {'id': id_ticket}).fetchone()
                    
                    if ticket_resuelto_data:
                        notificar_ticket_resuelto({
                            'IdTicket': id_ticket,
                            'NumeroTicket': ticket_resuelto_data[0],
                            'Titulo': ticket_resuelto_data[1],
                            'EmailCreador': ticket_data_result[1]
                        }, url_base)
        except Exception as e:
            current_app.logger.error(f'Error enviando notificación de cambio de estado: {str(e)}')
...
def mis_tickets_asignados():
    try:
        ...
        tickets_result = db.session.execute(text("""
            SELECT * FROM v_tickets_asignados WHERE IdAsignadoA = :tecnico_id
        ...
        return render_template('ticketing/mis_asignados.html', tickets=tickets, total_tickets=total_tickets,
                             abiertos=abiertos, en_proceso=en_proceso, escalados=escalados, resueltos=resueltos)
    except Exception as e:
...
def dashboard_estadisticas():
    try:
        deptos = db.session.execute(text("""
            SELECT d.nombre as NombreDepartamento, COUNT(t.IdTicket) as total
            FROM Tickets t
            INNER JOIN usuario u ON t.IdUsuarioCreador = u.id
        ...
        meses = db.session.execute(text("""
            SELECT DATE_FORMAT(FechaCreacion, '%Y-%m') as mes, COUNT(IdTicket) as total
            FROM Tickets WHERE FechaCreacion >= DATE_SUB(NOW(), INTERVAL 6 MONTH)
            GROUP BY mes ORDER BY mes
        """)).fetchall()
        
        categorias = db.session.execute(text("""
            SELECT Categoria, COUNT(*) as total FROM v_tickets_dashboard
            GROUP BY Categoria ORDER BY total DESC LIMIT 8
        """)).fetchall()
        
        tiempos = db.session.execute(text("""
            SELECT d.nombre, ROUND(AVG(TIMESTAMPDIFF(HOUR, t.FechaCreacion, t.FechaCierre)), 1) as horas
            FROM Tickets t
            INNER JOIN usuario u ON t.IdUsuarioCreador = u.id
            INNER JOIN Departamentos d ON u.IdDepartamento = d.IdDepartamento
            WHERE t.FechaCierre IS NOT NULL
            GROUP BY d.nombre ORDER BY horas LIMIT 10
        """)).fetchall()
        
        return jsonify({
            'tickets_por_departamento': {'labels': [r[0] for r in deptos], 'data': [r[1] for r in deptos]},
            'tickets_por_mes': {'labels': [r[0] for r in meses], 'data': [r[1] for r in meses]},
            'tipos_problema': {'labels': [r[0] for r in categorias], 'data': [r[1] for r in categorias]},
            'tiempo_resolucion': {'labels': [r[0] for r in tiempos], 
